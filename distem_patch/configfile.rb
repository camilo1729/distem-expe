  ...
  class ConfigFile
    ...
    def self.generate(vnode,filepath)
      ...
      File.open(filepath, 'w') do |f|
        ...
        # creating phys interface
        vnode.vifaces.each do |viface|
          f.puts "lxc.network.type = phys"
          f.puts "lxc.network.hwaddr = #{Distem::Lib::NetTools.get_iface_mac('eth1')}"
          f.puts "lxc.network.link = eth1"
          f.puts "lxc.network.flags = up"
          f.puts "lxc.network.ipv4 = #{viface.address.to_string}"
          f.puts "lxc.network.ipv4.gateway = #{Distem::Lib::NetTools.get_default_gateway}"
        end
